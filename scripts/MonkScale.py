...
def extract_filename_and_extension(url):
    """Izvlači ime fajla i ekstenziju iz URL-a"""
    path = Path(url.split('/')[-1])
    return path.stem, path.suffix

def alphabet_id(i):
    """Konvertuje broj u alfabetski identifikator (a, b, ..., z, aa, ab, ...)"""
    return chr(97 + i % 26) * (i // 26 + 1)

class ArgumentError(Exception):
    """Klasa za greške specifične za ovaj modul"""
    pass
